---
title: "Projeto Integrador: Predizendo o número de inscritos em canais do Youtube"
...
setwd("C:/Users/Avell/Google Drive/Luiz Gabriel - PC/13.Pós Data Science UFPR/02.Inferencia_Estatistica/Modulo_01/Projeto_Integrador")
```
<hr>

### 1 - Carregando a base de dados e os pacotes necessários:
```{r}
library(ggplot2)
library(ggthemes)
library(tidyr)
...
### 2 - Separando os dados por canal:
```{r}
...
### 3 - Calculando o número acumulado de inscritos para cada canal:
```{r}
...
#### 5.2 - Gráfico do número acumulado de inscritos por dia:
```{r}
title.c1 = "Canal Invento na Hora"

cum.plot <- function(df, x, y, title) {
  plot <- ggplot(df, aes(x = x)) +
    geom_line(aes(y = y), colour = "darkseagreen3", size = 2) +
...
#### 5.3 - Gráfico do número de inscritos por dia:
```{r}
qty.plot <- function(df, x, y, title) {
  ggplot(df, aes(x = x)) +
    geom_point(aes(y = y/100), color = "darkseagreen3", alpha = 0.5, size = 2) +
...
#### 5.4 - Preparando um data frame com o número total de dias que será alimentado com os valores preditos:
```{r}
...
##### 5.6.1 - Criando um grid de combinações em torno dos parâmetros encontrados manualmente no tópico anterior para serem testados:
```{r}
...
##### 5.6.3 - Encontrando perda quadrática para cada uma das combinações testadas e selecionando a menor:
```{r}
grid.c1$SQ <- NA
for(i in 1:nrow(grid.c1)) {
...
##### 6.6.1 - Criando um grid de combinações em torno dos parâmetros encontrados manualmente no tópico anterior para serem testados:
```{r}
...
##### 6.6.3 - Encontrando perda quadrática para cada uma das combinações testadas e selecionando a menor:
```{r}
grid.c2$SQ <- NA
for(i in 1:nrow(grid.c2)) {
